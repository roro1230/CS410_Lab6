...
def get_ds(config):
    ...
    ds_raw = load_dataset(f"{config['datasource']}/vi_en-translation", split='train')
    ...
    # Tìm độ dài tối đa của mỗi câu trong câu nguồn và câu đích
    max_len_src = 0
    max_len_tgt = 0

    for item in ds_raw:
        src_text = item[config['lang_src']]
        tgt_text = item[config['lang_tgt']]
